    ...
    return(
        ...
        <div className={styles.menu__wrapper}>
            <div className={styles.menu__button}>
                <NavLink to="/" className={({ isActive }) => (isActive ? styles.active : "")}><img src={home__icon} alt="Home"/></NavLink>
                <button ref={target} className={`${tooltip_show ? styles.active : ""}`} onClick={() => dispatch({type:"toggle"})}><AiFillPlusCircle/></button>
                <NavLink to="/transactions" className={({ isActive }) => (isActive ? styles.active  : "")}><img src={transaction__icon} alt="transaction"/></NavLink>
            </div>

            <Overlay target={target.current} show={tooltip_show} placement="top">
                {(props) => (
                    <Tooltip id="tooltip-top" className={`${styles.menu__nft__overlay}`} {...props}>
                        <button onClick={createNftModal}><span><img src={create__icon} alt="Create NFT"/></span> Create NFT</button>
                        <button onClick={sendNftModal}><span><img src={send__icon} alt="Send NFT"/></span> Send NFT</button>
                    </Tooltip>
                )}
            </Overlay>
        </div>
